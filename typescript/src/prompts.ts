// The ONE system prompt + tool definitions + research-pipeline prompts/schemas.
// Ported from python/prompts.py. Tool schemas use the Anthropic-native shape
// (name / description / input_schema); TS is Anthropic-only so there's no
// OpenAI wrapping path.

export function systemPrompt(travellerEmail: string): string {
  return `You are a friendly, knowledgeable travel planning assistant.

The traveller you are helping is signed in as: ${travellerEmail}
    ...
    input_schema: {
      ...
      properties: {
        query: {
          type: 'string',
          description:
            "A focused, self-contained question, e.g. '5 days in Tokyo for first-timers who love food'",
        },
      },
      required: ['query'],
    },
...
// ─────────────────────────────────────────────────────────────────────────────
// research_destination pipeline prompts + JSON schemas (structured outputs).
// ─────────────────────────────────────────────────────────────────────────────

export function planSystem(count: number): string {
  return `You are a travel research planner. Given a research brief about a destination or trip, produce a set of focused web searches that together will answer it.
...
export const SEARCH_SYSTEM =
  'You are a travel research assistant. Use web search to investigate the given ' +
  'query, then write a concise, factual summary (1–2 paragraphs, under 250 words) ' +
  'of the most relevant findings. Capture specific facts: place and neighborhood ' +
  'names, signature dishes, prices, seasons, and travel times. Ignore fluff. Do ' +
  'not add commentary about your process.';

export const WRITE_SYSTEM =
  'You are a senior travel writer. You are given a research brief and a set of ' +
  'summarized findings from independent web searches. Synthesize them into one ' +
  'cohesive destination guide.\n\n' +
  '- markdown_report: a tight, well-structured guide in Markdown (headings, short ' +
  'paragraphs, and a table of highlights — e.g. attractions, neighborhoods, or ' +
  'costs — where useful). Aim for 250–400 words. Ground every claim in the ' +
  'findings; do not invent facts.\n' +
